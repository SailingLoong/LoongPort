...
## LoongPort 原生中转站质量与成本观测（2026-08-13 记）

**what**：中转站广场当前展示 VeriDrop 的公开评分证据。后续需要基于 LoongPort 自己的
真实接入链路建立可持续观测，覆盖：
...
- 结合倍率与模型价格后的有效价格
- 一段时间内的质量和性能趋势
- Claude、OpenAI、Gemini 各协议实现的一致性
- 采样产生的真实调用成本
- 请求内容、日志保留与站点可见范围等隐私边界

**why 现在不做**：这些指标需要先定义可比较的采样方法、成本上限和隐私规则；在数据量、
覆盖面与稳定性不足时对外给出 LoongPort 分数，会把临时噪声包装成可信排名。当前广场已经有
公开且可追溯的 VeriDrop 证据，不应同时维护第二套未成熟评分，造成两个权威来源。

**how-to-repay**：

1. 先定义观测口径、采样频率、失败分类、成本预算与隐私政策，并确认不同协议之间哪些指标
   可以直接比较。
2. 建立只记录原始观测与可审计聚合结果的采集链路，验证采样不会泄露用户请求内容或凭据。
3. 用足够长的灰度期验证覆盖率、稳定性、趋势一致性和异常站点处置，再决定是否形成 LoongPort
   自有评分。
4. **在原生数据达到生产可用且完成产品验收前，VeriDrop 始终是中转站评分、排名、样本数与
   检测日期的唯一权威来源；LoongPort 不对同一事实另算或镜像一份分数。**
