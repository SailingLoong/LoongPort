...
把 LoongPort 当作**你自己站点的客户端**发给新用户 —— 注册、充值、拿密钥都发生在你的站点上，新用户从下载到能用只剩「填域名、登录」两步。它是通用的 sub2api / new-api 客户端，不绑定任何一家站点：
...
它替你完成的三件事，每一条均可在源码中核验：`normalize_site_origin`（`relay/sub2api.rs`，地址栏整条粘贴的域名亦可识别）、落至你自己注册页的自助注册（`relay/login.rs`，新站 `/register`、老用户 `/login`，邀请码随 URL 附带）、注入登录态的自助充值（`relay/purchase.rs`，打开 `{你的域名}/purchase`）。
...
4. **点击要使用的档位** —— Codex 使用 OpenAI 系档位、Claude 使用 Anthropic 系档位，点击该档位的「设为当前」即可写入对应配置（`~/.codex/config.toml` 或 Claude 的 settings），随后 Codex 或 Claude Code 即可直接运行。
...
凭据与站点信息在本机 `~/.loongport/` 中**加密保存**（明细与边界见[数据与隐私](#数据与隐私)），调用时作为 Bearer token 发送给你选定的中转站。LoongPort 没有账号体系、没有服务端，无法获取你的凭据。
...
- **档位看板** —— 开启后主页变为档位看板，每个档位的倍率、预估单价、余额、首字耗时与当前命中一目了然，并整合[模型验证](#模型验证)的异常标记。
- **手动优先级** —— 不希望交给系统时，可切换为手动排序，拖拽调整档位优先级，松手即生效；会话保持与故障转移照常保留。
- **随时开关** —— 顶栏常驻开关可随时开启或关闭当前应用。开启时会一并开启本地路由并接管该 CLI 的配置，关闭时自动恢复原有配置。

省心模式的开关在**应用页顶部**；总开关是**设置 → 高级 → 本地路由**——打开路由后，应用页顶部才会出现常驻开关（该分区里也有「在主页面显示本地路由开关」可控制它是否显示）。不开启时一切保持原状：手动切换与故障转移队列照常可用，底层路由配置同样收在设置「高级」页。
...
- **凭据在本机加密保存。** API 密钥、OAuth 令牌、登录态、档位配置与 MCP 配置里携带的凭据，落在 `~/.loongport/` 的数据库与凭据文件中的都是密文；自动备份与云同步快照同样按这套保护处理。默认由**系统凭据库**解锁（macOS 钥匙串 / Windows 凭据管理器 / Linux Secret Service），不需要记口令；也可以自设保护口令，用于恢复数据和跨设备同步。
- **有一处仍然是明文：写进下游 CLI 的配置。** Codex 要读 `~/.codex/config.toml`、Claude 要读它自己的配置，工具要能直接使用，那些文件就只能可读——这部分靠收紧文件权限降低暴露面，而不是靠加密。同样地，调用上游时凭据必须发给你选定的站点，所以不存在「密钥永不离开本机」。
- **使用记录仅存本机。** 经本地路由的请求数、成本与 token 统计不出本机，扣费对账同样只在本地完成。
- **两处上报，都可以随时关闭。** 一是**安装统计**：站点域名、站点个数、应用版本、操作系统，以及一个随机生成、不含硬件指纹的安装标识（它的用途是去重，因此是持久的）；二是**站点实测数据共建**：按「站点 × 小时」聚合的首字耗时、错误率、token 与花费，来源标识每日轮换。两者都**不含**提示词、请求体、账号身份与密钥。新建安装在引导里确认一次——**分享选项默认已勾选**，取消勾选即可不参与；老版本升级上来的保留原有选择。开关在**设置 → 通用 → 窗口行为**。
- 准确的词是**假名化**，不是「匿名」：它不含直接身份字段，但安装标识是持久的，冷门站点的组合本身可能构成准标识符。另外，省心模式使用的本地路由**只监听 `127.0.0.1`**，不对局域网或公网开放。
...
**Windows Setup 安装版、macOS 版与 Linux AppImage 会自动检查更新**：启动数秒后在后台查询一次，有新版本时在「设置 → 关于」中提示，点击即可下载、安装并重启。检查失败不会造成打扰（离线、无法连接 GitHub 的情况均属常见），也可随时手动点选「检查更新」。
