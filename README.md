...
只提供生图模型的档位上会多一个「启用生图」按钮。点一下，LoongPort 就把内置的生图工具
（一个 MCP server）注册进 Codex、Claude Code 与 Gemini CLI —— 之后在对话里说
「生成一张图」即可。

三件值得知道的事：

- **对话档位不必让出来。** 聊天走 `/v1/responses` 用当前档位的密钥，生图走
  `/v1/images/generations` 用生图档位的密钥 —— 两个「当前项」各自独立，切换任一个都
  不影响另一个。你照旧用便宜的文本档位聊天。
- **换生图档位不用重启 CLI。** MCP 条目里不存档位 id，选择存在 LoongPort 自己的库里、
  每次生图时现读。只有**第一次启用**需要新开一个终端（那时才往 CLI 配置里加条目，
  而 CLI 只在启动时读配置）。
- **没有生图分组的站点完全不显示这些**，你的 CLI 配置一个字不动。而 1K 与 4K 档位
