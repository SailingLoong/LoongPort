...
- **站点实测数据共建默认参与。** 各站点的实测指标（首字耗时、错误率、token 用量等）按小时聚合后匿名共享，用于生成应用内广场与[状态页](https://loongport.dev/status)的各站实测数据。共享内容**不含**提示词、密钥或账号信息，也不含可识别个人的标识（共享用的随机标识每日轮换）。
- 首次添加中转站后会有一次明确告知，在完成告知之前不会有任何数据上传；不想参与可随时在**设置 → 窗口行为 → 站点实测数据共建**关闭。
