...
- **两处上报，都可以随时关闭。** 一是**安装统计**：站点域名、站点个数、应用版本、操作系统，以及一个随机生成、不含硬件指纹的安装标识（它的用途是去重，因此是持久的）；二是**站点实测数据共建**：按「站点 × 小时」聚合的首字耗时、错误率、token 与花费，来源标识每日轮换。两者都**不含**提示词、请求体、账号身份与密钥。新建安装在引导中由你明确选择一次才参与，老版本升级上来的保留原有选择。开关在**设置 → 通用 → 窗口行为**。
