...
> **macOS 首次打开会被拦一下 —— 按下面的顺序做，一次就好。**
>
> macOS 版未做 Apple 代码签名与公证，所以 Gatekeeper 会报「已损坏，应移到废纸篓」
> —— **不是真的损坏**，是缺签名。**顺序很重要**，四步：
>
> 1. **先退出正在运行的 LoongPort**（如果之前装过）。点菜单栏/托盘的图标 → 退出，
>    或按 <kbd>⌘</kbd><kbd>Q</kbd>。**不退出的话第 2 步会覆盖不了那个正在跑的程序。**
> 2. **打开 dmg，把 LoongPort 拖进「应用程序」—— 但先别打开它。**
>    这时候双击它，得到的就是那句「已损坏」。而**被系统弹过一次之后有些机器会更麻烦**，
>    所以先装好、放着。
> 3. **打开「终端」（聚焦搜索里搜 Terminal），粘贴这一行回车**：
>
>    ```bash
>    xattr -dr com.apple.quarantine /Applications/LoongPort.app
>    ```
>
>    没有任何输出就是成功了（Unix 命令的惯例：没消息就是好消息）。
>    要是提示 `No such file or directory`，说明第 2 步没拖进「应用程序」——
>    回去确认一下那个文件夹里有 `LoongPort`。
> 4. **现在打开它。** 之后每次打开都正常，**这四步只做这一次** ——
>    直到你装下一个版本（那时从第 1 步再来一遍）。
>
> <details>
> <summary>这条命令到底做了什么（点开看）</summary>
>
> 从浏览器下载的文件，macOS 会给它打一个叫 `com.apple.quarantine` 的隔离标记。
> Gatekeeper 见到这个标记就去查签名，查不到就拒绝运行 —— 而对未签名的应用，
> 它给出的措辞恰好是最容易让人误解的那句「已损坏」。
>
> `xattr -dr com.apple.quarantine` 就是**把那个标记去掉**（`-d` 删除、`-r` 递归到
> 包内所有文件）。它不改动程序本身，也不关闭系统的任何安全设置 —— 只是对**这一个**
> 应用说「我知道它来自哪里」。
>
> 所以这条命令的安全性完全取决于**你信不信这个来源**。请只对从
> [本项目的 Releases 页](../../releases)下载的包执行它。
>
> </details>
>
> 签名与公证需要 Apple 开发者账号（$99/年），会在后续版本补上 —— 补上之后这四步会
> 缩成「拖进应用程序，双击」。它影响的只是安装这一步，装好之后功能与 Windows 完全一致。
...
| 用户要做的 | 照常规做法 | 用 LoongPort |
|---|---|---|
| 拿到可用的 CLI | 注册 → 找控制台 → 手建 key → 抄 base_url → 找配置文件 → 写对字段 → 换档位再来一遍 | 粘一个域名 → 在你的站上注册 → 点一下档位 |
| 充值 | 自己找入口、重新登录 | 点余额旁边的按钮，已是登录态 |

三件它替你做的事，每条都可以在源码里核：

- **一个域名直达** —— 从浏览器地址栏整条粘过来也行，`https://`、`www.` 与后面的路径都会
  自动去掉（`operator/api.rs` 的 `normalize_site_origin`）。
- **自助注册，落在你自己的注册页** —— 打开的是你站点真实的页面而非模仿的表单；新站落
  `/register`、已登录过的落 `/login`，邀请码与优惠码随 URL 带上（`operator/login.rs`）。
- **自助充值，不用再登录一次** —— 点余额旁的按钮直接开 `{你的域名}/purchase`，登录态已注入
  （`operator/purchase.rs`）。余额低于 $5 时客户端会主动提醒用户。

**你不会失去什么**：LoongPort 没有账号体系、没有服务端，不经手流量也不经手钱。用户注册的是
你的站、充值付给你，凭据只存在他自己电脑的 SQLite 里。客户端只把配置写对，请求由用户的 CLI
直接打到你的接口。

**想让用户默认看到你的站**：客户端会拉一份带签名的远端配置，里面能带「推荐站点」列表 ——
它出现在「选择服务站点」那一屏最上面，用户点一下就连上，不必自己粘域名；同一份配置里也能带
你的邀请码与优惠码。[开个 issue](../../issues) 说一声即可。

完整说明（含技术前提与接入步骤）：**[loongport.dev/zh/for-relays](https://loongport.dev/zh/for-relays)**
