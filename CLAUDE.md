...
### 版本号升哪一位（patch / minor 判据，2026-09-16 定）

**默认 patch**（`6.25.1`、`6.25.2` …）：修复与小改动一律从最小的位开始加，
**小功能也走 patch**。**只有内容成规模才开 minor**——新功能成批、或发版前
明确判定「这版内容比较多」（先例：v6.16.0 用户点名升 minor）。minor 是攒出来
的，不是每批的默认动作。2026-09-16 用户点名立规；此前 6.19.0–6.24.0 连续
开 minor 属于流程漂移，勿再模仿。beta 段同判据：patch 量级的新段从
