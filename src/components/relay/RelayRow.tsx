...
function TierItem({
  ...
  return (
    ...
    >
      ...
      {/* 整组（含主按钮「启用 / 使用中」）hover / focus 才出现 —— **信息常驻、
          动作按需出现**（2026-09-13 用户定调：档位多时每行常驻按钮全是噪音；
          #400 曾把主按钮提出组外常驻，当天按同一定调收回）。「在用」态由行的
          蓝色边框+淡底表达，不靠按钮。

          `pointer-events-none` 不能省、两个 group 为什么取名字，
          见文件头 `ROW_HOVER_ACTIONS` 那段注释。进行中的操作（含 switching ——
          主按钮的转圈也在组里）钉住可见：鼠标一移开还知道自己在等什么。 */}
      <div
        className={cn(
          "flex flex-shrink-0 items-center gap-0.5",
          HOVER_ACTIONS_BASE,
          checking || resetting || switching || modelSwitching || verifying
            ? HOVER_ACTIONS_PINNED
            : TIER_HOVER_ACTIONS,
        )}
      >
