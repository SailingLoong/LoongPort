  ...
  if (account.status === "ready" || account.status === "sessionExpiredUsable") {
    return (
      <div className="flex shrink-0 items-center gap-2">
        {/* 整组 hover / focus 才显形 —— **信息常驻、动作按需出现**（2026-09-13
            用户定调；#400 曾改主按钮常驻，当天收回）。「在用」态靠行的蓝色边框
            表达；进行中钉住可见。唯一同组的「重登」是登录过期状态的出口。 */}
        <div
          className={cn(
...
function PlanItem({
  ...
  return (
    ...
    >
      ...
      {/* 整组（含主按钮「使用 / 在用」）hover / focus 才出现 —— 信息常驻、动作
          按需出现（2026-09-13 用户定调；#400 曾改主按钮常驻，当天收回）。
          `pointer-events-none` 不能省（透明按钮仍可点），进行中钉住。 */}
      <div
        className={cn(
