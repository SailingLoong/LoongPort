 ...
 *
 * 2026-08-07 从 `OperatorTierList` 拆出来：原来两类行（中转站 / 官网）混在同一个
 * dnd 列表里，现在各归各的区块 —— 中转站行归 `OperatorTierList`，官网行走这里。
 *
 * ## 区块头
 *
 ...
 *
 * ## 与 `OperatorTierList` 的边界
 *
 * 排序、余额、busy 全部沿用 `OperatorTierList` 的既有模式：
 * - dnd id 仍是判别式 `RowKey`（`"vendor:3"`），虽然这里只有一个类型，但
 *   `openState` / `balances` 的键与中转站那块的 `RowKey` 同一套，别退回裸 number。
 * - 余额是**已格式化的字符串**（`"¥547.08"`，后端给的），与 operator 那条
...
export function VendorBlock({ vendor, busy, onAddVendor }: VendorBlockProps) {
  ...
  // 与 `OperatorTierList` 同一个 sensor 配置 —— 视觉与手感一致。
...
/** 给 `VendorRow` 套一层 dnd-kit 的 sortable 壳（与 `OperatorTierList` 里那个逐字同形）。 */
