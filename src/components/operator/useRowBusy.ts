 ...
 * B / C 的按钮全灰了**（他明确指出过：运营商之间、账号之间本来没有依赖）。
 *
 * 那个全局禁用当时是在兜一个**真实的并发正确性问题**：后端 `operator_provision`
 * 靠 `creds::load()` 读「`is_current = 1` 的那一行」，前端得先 `set_current(id)`
 * 才能让它作用到对的账号上 —— 而 `is_current` 是全局单例，两个 provision 并发时
 * 会互相改写对方的目标。
 *
 * 根因已经修掉（命令改吃 `operator_id`，全局状态不再参与定位），所以禁用可以
 ...
 * 所以 `OperatorRow` 里那些 `busy === \`login:${operator.id}\`` 的判断不用改。
 */
