 ...
 */
const OPERATOR_ROW_TSX = path.resolve(
  __dirname,
  "..",
  "..",
  "src",
...
const source = fs.readFileSync(OPERATOR_ROW_TSX, "utf8");

describe("OperatorRow hover-reveal actions", () => {
  ...
  it("uses named groups so the outer row does not reveal every nested tier", () => {
    // 档位行**嵌在**运营商行里。裸 `group-hover:` 编译成 `.group:hover &`，
    // 会匹配任意带 `group` 的祖先 ⇒ 鼠标停在运营商行上就把里面所有档位行点亮。
