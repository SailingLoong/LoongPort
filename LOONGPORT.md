...
这两句是**固定副标题**，README 与官网的第一句都用它，别每处另写一版。
产品名只承担识别（同 Docker / Vercel，不自我描述），"它是什么"由副标题讲 ——
少了这句，`Port` 容易被读成"移植版"，正好是最不想要的解读。

⚠️ **视角是用户的（我填个域名就能用），不是实现的。** 曾经有一版写「一个中转站
账号，直接变成任意 AI CLI 的可用供应商」—— 那句描述的是软件内部干了什么，而用户
关心的是自己得到什么。2026-09-02 起副标题又从省钱口径换成一键口径：主要用户群是
中转站用户，主卖点是「接入与使用的全程不用碰配置」。**2026-09-05 省钱/价格叙事
整体下线**：对外只讲「方便省心」，线上不再出现省钱卖点与「5% / 20%」推导——
README 双语「为什么便宜这么多」推导转 HTML 注释（充值与返利披露保留可见，节名
「充值与返利」/「Payments and referral rebates」）；官网 /pricing 摘除路由与导航/
页脚入口、首页 SavingSection 不再渲染、features 末条与 FAQ 末两条注释；教程仓
「能省多少钱」整节与 5% 钩子注释、About 描述去 5%；llms.txt 去比价句。**全部内容
保留在源码**（组件/文案/注释原样），恢复 = 各处反向取消注释。副标题同步点：
`README.md` / `README_EN.md` 标题下方、官网 `src/i18n/translations.ts` 的 `TAGLINE`、
官网 `index.html`（`<title>` / `og:title` / `<noscript>`）与 `public/llms.txt`。

⚠️ 下面两条管的是「5% / 20%」这两个数字的写法。**2026-09-05 起线上已无出现点**，
规则仅在**恢复后**适用：

- **一律用「只花官方的 N%」这个 level 口径，别改回「省 N%」那个 delta 口径**
  （两者等价：花 5% ⟺ 省 95%）。选 level 的三条理由：
  1. **与官网 /pricing 的推导同单位** —— 那页算出来的就是「成本约为官方的 1.5%」。
     说 delta，读者从钩子走到证明要自己心算一次 95% ↔ 1.5%。
...
- **两个数字别合并成一个**（如笼统取上界写「只花 20%」）：Codex 与 Claude 的成本
  来源不同 —— Codex 吃到档位倍率折扣（×0.1）与汇率两层，Claude 只吃到汇率那层
  （Anthropic 分组无倍率折扣）。取上界砍掉 Codex 卖点的一半，取下界则让 Claude
  用户按 5% 预期然后落空。
