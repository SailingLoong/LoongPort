...
use cc_switch_lib::{AppError, MultiAppConfig, APP_DIR_NAME};
...
fn cfg_path() -> PathBuf {
    ...
    PathBuf::from(home).join(APP_DIR_NAME).join("config.json")
}
...
fn load_v1_config_returns_error_and_does_not_write() {
    ...
    let bak = home.join(APP_DIR_NAME).join("config.json.bak");
