...
fn has_explicit_codex_third_party_upstream(provider: &Provider) -> bool {
    ...
    ["baseUrl", "baseURL", "base_url"]
        ...
        || config
            ...
            .is_some()
        || config
            // toml 1.0 起 `Value::from_str` 只收单个值字面量，不收整段文档 ——
            // 文档解析必须走 `toml::from_str::<Table>`（2026-08-19 上游测试照出的
            // 静默坑：bump toml 1.0 后这个分支一直 parse 失败当不存在）。
            .and_then(|text| toml::from_str::<TomlTable>(text).ok())
            .and_then(|doc| {
...
fn extract_codex_wire_api_from_toml(config_text: &str) -> Option<String> {
    let doc = config_text.parse::<TomlTable>().ok()?;
