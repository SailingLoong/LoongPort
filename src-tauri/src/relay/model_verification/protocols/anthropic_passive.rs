            ...
            Some("content_block_delta") => {
                    ...
                    .and_then(Value::as_str)
                {
                    ...
                    Some("signature_delta") => {
                        self.record(EvidenceCode::ThinkingSignature, EvidenceOutcome::Passed)
                    }
                    _ => {}
                }
            }
...
mod tests {
    ...
    fn run(split: usize) -> serde_json::Value {
        let mut tap = AnthropicPassiveTap::new(TargetKey::new("p", "claude", "claude-sonnet-5"), 3);
    ...
    fn foreign_protocol_and_self_identification_are_sanitized_facts() {
        let mut tap = AnthropicPassiveTap::new(TargetKey::new("p", "claude", "claude-sonnet-5"), 1);
