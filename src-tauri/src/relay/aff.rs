...
//!
//! 编译进二进制的字符串 `strings` 一下就能看到 ⇒ 表里的码
...
//! 两条**有意不进表**的（进了也是死数据）：
//!
//! - **`bestapi.store` 有意不在表里**（缺席由下方测试钉住，别去「补齐」）。
...
const AFF_CODES: &[(&str, &str)] = &[
    ...
    // ⚠️ bestapi.store 有意不在表里（见模块文档）。
];

/// 查这个站有没有我们的 aff 码。
///
/// `None` = 表里没有 ⇒ 调用方**什么都不做**（不写那个键、不改 URL）。
/// 绝大多数站都会走这条路 —— 但**默认站在表里、有码**。
///
...
mod tests {
    ...
    #[test]
    fn bestapi_is_deliberately_absent_from_the_table() {
        // ⭐ 这条钉住**有意的缺席**，不是遗漏 —— 别给它「补齐」。
        //
        // ⚠️ 别把这个缺席推广到「默认站」：默认站**必须**在表里有码，
        // `commands::relay` 的 `DEFAULT_SITE` 那里有闸钉着。
