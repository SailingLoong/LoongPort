...
fn require_enabled_s3_settings() -> Result<S3SyncSettings, String> {
    let settings = settings::get_s3_sync_settings()
        .map_err(|error| error.to_string())?
        .ok_or_else(s3_not_configured_error)?;
    if !settings.enabled {
...
pub async fn s3_test_connection(
    ...
    let existing = settings::get_s3_sync_settings().map_err(|error| error.to_string())?;
    let resolved = resolve_secret_for_request(settings, existing, preserve_empty);
...
pub async fn s3_sync_restore(
    ...
    let snapshot = s3_sync_service::fetch_snapshot(&settings)
        .await
        .map_err(|error| error.to_string())?;
    ...
    .await?;
    s3_sync_service::persist_download_success(&mut settings, &snapshot);
    Ok(result)
}
...
mod tests {
    ...
    fn persist_sync_error_updates_status_without_overwriting_credentials() {
        ...
        let after = crate::settings::get_s3_sync_settings()
...
#[tauri::command]
pub async fn s3_sync_legacy_cleanup_preview(
    state: State<'_, AppState>,
) -> Result<crate::services::sync_cleanup::LegacyCleanupPreview, String> {
    let settings = require_enabled_s3_settings()?;
    settings.validate().map_err(|error| error.to_string())?;
    let remote = s3_sync_service::LegacyCleanupRemote(&settings);
...
#[tauri::command]
pub async fn s3_sync_cleanup_legacy(
    state: State<'_, AppState>,
