...
impl SkillService {
            ...
            match Self::require_valid_directory(&skill.directory) {
                Ok(directory) => {
                    ...
                    for app in AppType::all() {
                        ...
                        Self::remove_from_app_preserving(
                            &directory,
                            &app,
                            preserved_pi_path.as_deref(),
                        )
                    ...
                    Some((h, freshly_computed)) => {
                        if freshly_computed {
                            db.update_skill_hash(&skill.id, &h, 0).warn_on_err(
                                DiagnosticEvent::new("skill.update_check", "hash_persist_failed")
    ...
    pub async fn update_skill(&self, db: &Arc<Database>, skill_id: &str) -> Result<InstalledSkill> {
        ...
        // （上游此处吞掉备份错误继续更新；LoongPort 保留「备份失败即中止」。）
    ...
    pub fn migrate_storage(
        ...
        for app in AppType::all() {
            Self::sync_to_app_unlocked(db, &app).warn_on_err(
...
mod tests {
    ...
    fn migrate_storage_retargets_an_equivalent_relative_pi_symlink() {
        ...
        let relative_target = format!("../../{}/skills/test-skill", crate::config::APP_DIR_NAME);
        std::os::unix::fs::symlink(Path::new(&relative_target), &pi_skill)
    ...
    async fn update_skill_persists_relocated_source_before_metadata_rename() {
        for location in [
            ...
            for (directory, old_path, new_path) in [
                ...
                let config_dir = home.path().join(".loongport");
