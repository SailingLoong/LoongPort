...
pub(crate) fn write_live_with_common_config_for_state(
    ...
    //
    // 本函数是**全部写 live 路径的收口**（switch、save、update、代理接管、
    // `sync_current_provider_for_app_to_live` … 全走这里或它委托的
    // `_codex_oauth_manager` 变体）。逐个加判断的话，漏一个就是一条真实的失败路径
    // —— 实测漏掉的正是「用户编辑一个当前生图档位并保存」：
