//! 远端配置：赞助运营商列表 + 注册邀请码，**Ed25519 签名，三层回落**。
//!
...
//!
//! **赞助运营商列表把这个权衡翻了过来**：谈成一家新的赞助商，不可能等下一个版本才让
...
/// 私钥只在维护者本机 `~/Documents/loongport-keys/`（**绝不进仓库**，离线签名），
/// 签名步骤见档案仓 `remote-config/sign.sh`。
///
...
///   （`commands/operator.rs` 里 5 处 DTO 都是）。
///
/// 分方向 rename 是安全的，因为**缓存存的是原始字节**（`write_cache`），
/// 不经过 `Serialize` ⇒ 它唯一的消费者就是 [`super::super::commands::operator`]
/// 那个 `operator_list_sponsors` 命令。改这里不会影响验签或缓存。
...
mod tests {
    ...
    ///   （`src/lib/api/operator.ts` 里的类型全是），改了前端拿到 undefined。
    ///
