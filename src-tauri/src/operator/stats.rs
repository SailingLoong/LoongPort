...
//!
//! ⚠️ **绝不复用 `creds` 的 `device_id`。** 那个 id 被写进运营商服务器上的 API key 名
//! （`provision::key_name_for` → `LoongPort/<device_id>/<platform>/<group_id>`）⇒
//! 任何看得到它的运营商都能把一条上报**对回一个具体的付费账号**。
...
pub fn build_report(install_id: String, app_version: String, origins: &[String]) -> Report {
    let operator_account_count = origins.len();
...
mod tests {
    ...
    fn report_never_carries_credentials_or_account_fields() {
        ...
        // `operatorAccountCount` 是个**计数**（不含任何身份），它合法。
    ...
    #[test]
    fn operator_count_counts_rows_while_hosts_are_deduped() {
        ...
        assert_eq!(r.operator_account_count, 3, "个数是行数");
    ...
    fn a_user_with_no_sites_still_produces_a_valid_report() {
        ...
        assert_eq!(r.operator_account_count, 0);
