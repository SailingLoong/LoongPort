//! LoongPort 运营商接入层。
//!
//! 与 cc-switch 的关系：本模块是 fork 新增的，把「一个运营商账号」变成 CLI 可用的
//! provider 记录。
//!
//! ## 目标形态是多运营商 × 多 CLI，当前实现只覆盖一格
//!
//! | 维度 | 已实现 | 缺口在哪 |
//! |---|---|---|
//! | 运营商 | sub2api | [`api`] 的 DTO 与端点是 sub2api 形状。接 new-api 要在这一层分化出 provider trait |
...
//! - 命令层签名**都吃 `app_id`**（`commands::operator`），非 codex 明确报错而非静默走错分支
//! - [`creds`] 的 `login_identifier` 是**中立命名**（不叫 `account_email`），
//!   正是为 new-api 那种用 username 登录的运营商留的 —— 列名进了 schema，改它是迁移不是重构
//!
//! 所以扩展时改的是「加一份实现」，不是「拆掉一个假设」。
//!
//! 模块划分：
//!
//! - [`api`]：运营商的窄 DTO + HTTP 客户端（探测 / 分组 / Key / 余额 / 倍率）。**当前是 sub2api 形状**
