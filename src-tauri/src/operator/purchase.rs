//! 充值 WebView：把登录态注入进运营商的充值页，让用户不必再登一次。
//!
//! ## 与 [`super::login`] 方向相反
//!
...
//! `REFRESH_TOKEN_INVALID`（`:1565`）⇒ **用户被迫重新登录整个运营商**。
//!
...
//! | 选择 | 坏后果 | 触发条件 |
//! |---|---|---|
//! | 注入四个键 | 本仓的登录态被搞坏，**用户被迫重新登录整个运营商** | **必然发生**（站点一定会用掉那个一次性 refresh token） |
//! | 只注入两个 | 付款页可能中途掉登录态 | 只在「access token 剩余寿命撑不完这次付款」时 |
//!
//! 而后者已经被 `commands::operator::ensure_token_outlasts_a_payment` 压到很小：
...
/// 充值窗口 label 的前缀。**每个运营商一个自己的窗口**，label 是 `<前缀><operator_id>`。
///
...
/// 某个运营商的充值窗 label。
pub fn window_label(operator_id: i64) -> String {
    format!("{PURCHASE_WINDOW_LABEL_PREFIX}{operator_id}")
}
...
mod tests {
    ...
    #[test]
    fn window_labels_are_per_operator_and_collide_with_no_other_window() {
        // **按行分窗**是资损面的修法：全局单窗时「开新窗前销毁残留窗」会销毁一个
        // 可能正在付款的窗口（服务端订单不会因此取消，用户却失去确认页面）。
        assert_ne!(window_label(1), window_label(2), "不同运营商必须是不同窗口");
