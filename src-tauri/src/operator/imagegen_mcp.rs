...
///
/// 这个 crate 的 logger 由 `tauri_plugin_log` 在 [`crate::run`] 的 setup 里安装，
/// 而 MCP 模式**在 `run()` 之前就分流走了** ⇒ 这个进程里根本没有 logger ⇒
/// 所有 `log::` 宏都是**空操作**。原来那几行 `log::info!` 一个字都没落下来，
/// 而模块文档却宣称「诊断走 log（落文件）」—— 是最糟的状态：承诺了一条不存在的通道。
///
/// ## 为什么是 stderr 而不是自己装一个文件 logger
///
/// 1. **stderr 天然是 MCP server 的诊断通道**：宿主（codex / claude）会捕获子进程的
///    stderr 并落进它自己的会话日志，用户报问题时那份日志本来就要看 —— 比让他去翻
///    我们另一个目录里的文件更可能被找到。
/// 2. **它不在协议通道上**，所以没有污染 stdout 的风险（那是本模块最怕的事）。
/// 3. 自己装 logger 要么引新依赖，要么把 `tauri_plugin_log` 的初始化挪到 `run()` 之外
///    —— 后者正好会把 stdout target 带进 MCP 进程，即**制造**我们要防的那个故障。
macro_rules! diag {
    ($($arg:tt)*) => {
...
fn load_tier(provider_id: &str) -> Result<Tier, String> {
    ...
    // ⚠️ **`app_type` 必须参与查询** —— `providers` 的主键是 `(id, app_type)`，
    // 一个 `provider_id` **真的会有多行**（实测维护者库里 `loongport-vendor-…` 那条
    // 有 6 行：claude / claude-desktop / codex / hermes / openclaw / opencode）。
    //
    // 不带这个条件的后果：`query_row` 随机拿到其中一行，若拿到 claude 那行，
    // 下面 `extract_api_key(.., Codex)` 就读不出 `auth.OPENAI_API_KEY` ⇒ 报
    // 「配置里读不出密钥，请点获取密钥重新生成」—— 而那条建议**永远修不好它**
    // （重新 provision 只会再造出同样的多行）。而且失败与否取决于 SQLite 的返回顺序，
    // 无法复现。（review 抓出）
    //
    // 取 codex 是因为**写入方就是按 codex 找的**
...
async fn generate_image(
    ...
    // 恰好回 base64。实测那个档位是回的，但那只说明那一个分组的行为
    // （上游 sub2api 的响应解析同时接受 `b64_json` 与 `url`，说明两种都可能出现）。
    //
    // 要 base64 而不是下 URL：省掉「再发一次 HTTP 去取图」那条路径，也就省掉它的
    // 超时、重定向、鉴权与临时链接过期这一整套失败面。
    let body = json!({
        "model": tier.model,
        "prompt": prompt,
        "n": 1,
