...
///
/// 末尾保留 `LoongPort` 标记：便于运营商在日志里认出本客户端，且不影响引擎指纹比对。
...
/// ## 为什么提成常量（登录窗只读、充值窗要写）
///
/// [`login_script`] 读它判断「登录完了没」，而 [`crate::operator::purchase`] 要**写**它
...
/// （`REFRESH_TOKEN_REUSED`）⇒ 用户被迫重新登录。见 [`crate::operator::purchase`]
/// 的模块文档。
...
/// 「成功登录后才写进去的东西」（`commands/operator.rs` 里 `op.login_identifier =
...
mod tests {
    ...
    /// （`commands/operator.rs` 里 `op.login_identifier = account.email`），
