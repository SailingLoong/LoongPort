...
//! ## 数据来源与边界
//!
//! 来自维护者的私有表（`~/Documents/邀请返利.xlsx`）。⚠️ **那份表含邀请主号邮箱与
//! 返利比例，是私有商业数据 —— 只把 host → code 两列搬进来**，其余留在表里。
//!
//! 两条**有意不进表**的（进了也是死数据）：
//!
...
///   所以存的可能是 `https://x.com:8443`。同一个站换个端口不该变成另一家运营商。
...
mod tests {
    ...
    #[test]
    fn port_is_ignored_so_one_site_is_not_two_operators() {
