...
impl Database {
    ...
    /// LoongPort seam：给「从本机 cc-switch 库整库搬过来」的导入用。
    ...
    {
        self.import_sql_with_validation_mode(sql_raw, preserve_tables, true, on_staging_ready)
    }

    fn import_sql_with_validation_mode<F>(
        &self,
        sql_raw: &str,
        preserve_tables: &[&str],
        strict_schema: bool,
    ...
    /// 只要求 providers 表 —— 见 [`Self::import_sql_string_from_cc_switch`] 的说明。
    fn validate_minimal_imported_schema(conn: &Connection) -> Result<(), AppError> {
        if !Self::table_exists(conn, "providers")? {
            return Err(AppError::localized(
                "backup.sql.invalid_schema",
                "导入的 SQL 缺少 providers 表".to_string(),
                "The imported SQL is missing the providers table".to_string(),
            ));
        }
        Ok(())
    }
