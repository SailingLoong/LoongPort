#!/usr/bin/env bash
#
# 验证现行世代（v2）config 的原始字节 Ed25519 签名。默认同时下载线上文件并与
# 本地进行字节比对；--local-only 仅验待发布文件，适用于签名后的本地检查。
#
# 端点从 remote_config.rs 的 CONFIG_URL 取（rc_const）—— 常量翻世代时本脚本
# 自动跟上，不存在"脚本验错世代"的分叉。v1 冻结世代的线上验证走 ./verify.sh。

set -euo pipefail

HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG="$HERE/public/v2/config.json"
SIG="$HERE/public/v2/config.json.sig"
RS="${LOONGPORT_REMOTE_CONFIG_RS:-$HERE/../src-tauri/src/relay/remote_config.rs}"
LOCAL_ONLY=false

if [ "$#" -gt 1 ]; then
  echo "用法：$0 [--local-only]" >&2
  exit 2
fi

case "${1:-}" in
  "") ;;
  --local-only) LOCAL_ONLY=true ;;
  *)
    echo "用法：$0 [--local-only]" >&2
    exit 2
    ;;
esac

# shellcheck source=lib.sh
. "$HERE/lib.sh"

require_openssl_with_ed25519

if [ ! -f "$RS" ]; then
  echo "✘ 找不到 remote_config.rs：$RS" >&2
  echo "  （用 LOONGPORT_REMOTE_CONFIG_RS 指定路径）" >&2
  exit 1
fi

PUBKEY_HEX=$(rc_const PUBLIC_KEY_HEX "$RS")

verify_local() {
  if [ ! -f "$CONFIG" ] || [ ! -f "$SIG" ]; then
    echo "✘ 缺 v2 config.json 或其签名；先跑 ./sign-v2-config.sh" >&2
    return 1
  fi
  if ! verify_signature "$CONFIG" "$SIG" "$PUBKEY_HEX"; then
    echo "✘ 本地 v2 config 签名验签失败；客户端会整份拒绝它。" >&2
    return 1
  fi
  validate_config_json "$CONFIG"
}

verify_local
echo "✔ 本地 v2 config 原始字节签名与契约均通过验证"

if [ "$LOCAL_ONLY" = true ]; then
  exit 0
fi

CONFIG_URL=$(rc_const CONFIG_URL "$RS")
SIGNATURE_URL=$(rc_const SIGNATURE_URL "$RS")
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT

curl -fsSL "$CONFIG_URL" -o "$TMP/config.json"
curl -fsSL "$SIGNATURE_URL" -o "$TMP/config.json.sig"

if ! verify_signature "$TMP/config.json" "$TMP/config.json.sig" "$PUBKEY_HEX"; then
  echo "✘ 线上 v2 config 签名验签失败；客户端必须拒绝它。" >&2
  exit 1
fi
if ! diff -q "$TMP/config.json" "$CONFIG" > /dev/null 2>&1; then
  echo "✘ 线上 v2 config 与本地待发布 JSON 不一致。" >&2
  exit 1
fi

echo "✔ 线上 v2 config 已通过签名验证，并与本地 JSON 字节一致"
