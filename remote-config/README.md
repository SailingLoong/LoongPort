...
2026-08-13 起，登录后取账号身份也走「reqwest 被 403 拦 ⇒ 由登录窗在页面上下文代拉」
的浏览器辅助回退（`commands/relay.rs::resolve_login_account_identity`），
`api.aijws.com` 这类站可以正常登录、显示账号名。仍受限的是**依赖 reqwest 的后续操作**
（余额展示、自动备 key）—— 它们拿不到浏览器指纹，会被同一个防护拦下。
