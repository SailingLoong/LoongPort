...
### preset_referral_urls：预设第三方厂商的返佣注册链接（可选键）

预设（表单里那批厂商）代码里带的是**中性注册链接**；与某家谈成返佣后，把
**完整注册 URL**（含那家的参数形状：`?aff=`、`/invite/`、`?ic=`…）录进这个
map 即可——客户端「获取 API Key」链接自动换用，**无需发版**：

```json
"preset_referral_urls": {
  "packyapi.ai": "https://www.packyapi.ai/register?aff=<我们的码>"
}
```

- key 规则与 `aff_codes` 同（小写、无 `www.`、纯 host）；客户端按「精确 host →
  去 `www.`」查，未命中回落代码里的中性链接
- 值必须 `https://` 开头（`lib.sh` 校验 + 客户端命令层双闸，坏值只敢不生效）
- 与 `vendor_invite_urls`（官网厂商那条）平行：那边管官方直连登录窗，这边管
  预设表单的注册链接
